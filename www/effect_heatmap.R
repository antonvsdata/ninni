...
discretize <- function(x, breaks, color_scale = "Sequential", midpoint = 0) {
...
plot_effect_heatmap <- function(data, log2_effect = FALSE, color_scale = "Sequential",
  ...
  legend_label <- effect_name
  if (log2_effect) {
    data$log_effect <- log2(data[, effect_name])
    effect_name <- "log_effect"
    legend_label <- paste0("log2(", effect_name, ")")
  }
  ...
  if (discretize_effect) {
    data$discrete_effect <- discretize(data[, effect_name], breaks, color_scale, midpoint)
    effect_name <- "discrete_effect"
  }
  ...
  if(length(vars1) > 80){
    vars2 <- levels(data$DS_Variable2)
    indx <- seq(1,length(vars1),length.out = 80)
    ggp <- ggp +
